...
Each time build.py is run it builds the configuration of the project specified in the 'BUILDCFG' build variable. The BUILDCFG variable can be set either by environment variable or by command line argument. For instance, the following two commands both have the effect of building the 'debug' configuration of the project:
```sh
...
### Key Value Pairs
Key value pairs are used to assign values to keys to define targets, configurations, and their properties in the build configuration file. The portion of the line before the first colon (:) is the key. Whitespace is stripped from the beginning and end of the key name. The portion of the line after the first colon is the value associated with that key. The value is parsed as a shell line and may potentially contain multiple words. Quotation marks or escapes may be used to escape whitespace, just as on a shell line.
```sh
key:value
